...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
  ...
  map
    .iter()
    .filter(|&(_, v)| v == &value) // Filter pairs where the value matches the given `value`
    .count() // Count how many matches there are
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
  ...
  //               { "variables2": Complete, … }, … ]
  collection
    .iter() // Iterate over the slice of HashMaps
    .flat_map(|map| map.iter()) // Flatten each HashMap into an iterator of key-value pairs
    .filter(|&(_, v)| v == &value) // Filter the pairs where the value matches the given value
