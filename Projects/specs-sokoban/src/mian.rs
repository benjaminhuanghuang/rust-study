...
use specs::{
  Builder, Component, DenseVecStorage, Join, NullStorage, ReadStorage, RunNow, System, VecStorage,
...
impl GameImages {
  fn new(ctx: &Context) -> Result<Self, GameError> {
    ...
    let _ = entity_images.insert(
      EntityType::PlayerUp,
      Image::from_path(ctx, "/Character7.png")?,
    );
    let _ = entity_images.insert(
      EntityType::PlayerDown,
      Image::from_path(ctx, "/Character4.png")?,
    );
    let _ = entity_images.insert(
      EntityType::PlayerLeft,
      Image::from_path(ctx, "/Character1.png")?,
    );
    let _ = entity_images.insert(
      EntityType::PlayerRight,
      Image::from_path(ctx, "/Character2.png")?,
    );
    let _ = entity_images.insert(
      EntityType::PlayerRightWalking,
      Image::from_path(ctx, "/Character3.png")?,
    );
    let _ = entity_images.insert(
      EntityType::Crate,
      Image::from_path(ctx, "/CrateDark_Beige.png")?,
    );
...
impl PlayerAnimation {
  fn walking(from: &Position, to: &Position) -> Self {
    let keyframes = keyframes![
      (
        [
          from.x as f32 * GAME_UNIT_SIZE,
          from.y as f32 * GAME_UNIT_SIZE
        ]
        .into(),
        0.0,
        functions::Linear
  ...
  fn get_image<'a>(&self, game_images: &'a GameImages) -> Option<&'a Image> {
    self.keyframes.as_ref().map(|keyframes| {
      ...
      if kf_now < 0.25 {
        game_images
          .entity_images
          .get(&EntityType::PlayerRightWalking)
          .unwrap()
      } else if kf_now < 0.5 {
        game_images
          .entity_images
          .get(&EntityType::PlayerRight)
          .unwrap()
      } else if kf_now < 0.75 {
        game_images
          .entity_images
...
impl<'a> System<'a> for SysMovePlayer {
  ...
  fn run(
    &mut self,
    (mut q, mut animation, mut positions, moverables, mut etypes): Self::SystemData,
  ) {
    for (pos, _mov, etype) in (&mut positions, &moverables, &mut etypes).join() {
      // player
...
fn main() {
  let (mut ctx, event_loop) = ContextBuilder::new("my_game", "Cool Game Author")
    ...
    .window_mode(WindowMode::default().dimensions(
      GAME_WIDTH as f32 * GAME_UNIT_SIZE,
      GAME_HEIGHT as f32 * GAME_UNIT_SIZE,
    ))
    .build()
  ...
  world
    ...
    .with(Moverable {})
    .with(EntityType::PlayerRight)
    .build(); // player
  world
    .create_entity()
    .with(Position { x: 10, y: 10 })
    .with(EntityType::Wall)
    .build(); // wall
  world
    .create_entity()
    .with(Position { x: 11, y: 11 })
