...
async function readMemoryWithOffsets(processId, baseAddress, offsets) {
  return await invoke("read_memory_with_offsets", {
    processId,
    baseAddress,
    offsets,
    size: 4,
...
async function writeMemoryWithOffsets(processId, baseAddress, offsets, data) {
  return await invoke("write_memory_with_offsets", {
    processId,
    baseAddress,
    offsets,
    data,
...
async function getModuleBaseAddress(processId, moduleName) {
  return await invoke("get_module_base_address", {
    pid: processId,
    moduleName,
  });
}
...
async function allocateMemory(processId, size) {
  try {
    const address = await invoke("allocate_memory_command", {
      processId: processId,
      size: size,
    });
    return address;
...
function getJumpByteArray(targetAddress, currentAddress) {
...
function App() {
  ...
  async function modeModify(isRead) {
    ...
    dataView.setUint32(0, goldValue, true);
    if (isRead) {
      const modeId = await readMemoryWithOffsets(
        processId,
        baseAddress,
        offsets
      );
  ...
  async function overPlant(modify) {
    ...
    if (modify) {
      ...
      secondByteArray = getJumpByteArray(firstAddress, secondAddress);
    ...
    >
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        >
          <TextField
            onChange={(event) => {
              setSunshineValue(event.target.value);
            }}
            value={sunshineValue}
            type="number"
            id="filled-basic"
        ...
        >
          <TextField
            onChange={(event) => {
              setModeIdValue(event.target.value);
            }}
            value={modeIdValue}
            type="number"
            id="filled-basic"
